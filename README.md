...
> * 명세서 만들기
> * 파일 다운로드 손보기
> * 그룹 페이지 내 공지사항, Task, 프로젝트, 일정(X) 제작
> * 공지사항 : 모델 새로 만듦. 모델 내 외래키로 그룹 지정해야 할 듯, 그룹 별 공지사항 다르기 때문에!
> * Task : 이미 있는 To do List 에 필드 추가해야 할 듯. 외래키? 그룹 Task도 개인 Task에 떠야 하기 때문에!
> * 프로젝트 : 버전관리인데, 필드에 title, body 추가
---
> ## 구현 현황
> 1. 로그인 이전 메인 페이지 (O)  
